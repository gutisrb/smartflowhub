...
import { getKandidatiByClientId, updateKandidat } from "@/lib/supabase/queries"
...
export function AgentLeadsModule({ clientId, terminology: propTerminology }: AgentLeadsModuleProps) {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel(`kandidati-rt-${clientId}`)
            .on("postgres_changes", { event: "*", schema: "public", table: "kandidati", filter: `client_id=eq.${clientId}` },
                () => loadCandidates())
            .subscribe()
        return () => { supabase.removeChannel(channel) }
    }, [loadCandidates, clientId])

    const filteredLeads = leads.filter(c => {
        const matchesSearch = c.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            c.posao?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            c.lokacija?.toLowerCase().includes(searchTerm.toLowerCase())
    ...
    const STATUS_OPTIONS = ['Novi', 'Prijavljen', 'Kvalifikovan', 'Intervju', 'Zaposlen', 'Intervencija', 'Odbijen']
        ...
        <div className="space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex items-center gap-4">
                <div className="relative w-full sm:w-80">
                    ...
                    <Input
                        placeholder={terminology.searchPlaceholder || "Pretraži kandidate..."}
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-white/5 border-b border-white/10">
                                <th className="p-4 text-xs font-mono uppercase tracking-widest text-emerald-400">Kandidat</th>
                                ...
                                <th className="p-4 text-xs font-mono uppercase tracking-widest text-emerald-400">Prijavljen</th>
                        ...
                        <tbody>
                            <AnimatePresence mode="popLayout">
                                {loading ? (
                                    ...
                                    <TableRow className="border-none">
                                        <TableCell colSpan={6} className="h-80 text-center">
                                            <p className="text-sm font-outfit text-zinc-500 italic">Nema kandidata</p>
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    filteredLeads.map((lead, idx) => {
                                        const kanal = getKanal(lead)
                                        const prijavljen = isPrijavljen(lead)
                                        return (
                                            <motion.tr
                                                key={lead.id}
                                            ...
                                            >
                                                ...
                                                <td className="p-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 border border-white/10 flex items-center justify-center text-emerald-400 font-bold shrink-0">
                                                            {lead.full_name?.[0] || 'K'}
                                                        </div>
                                                        <div>
                                                            ...
                                                            <div className="text-xs text-white/40 flex items-center gap-2 mt-0.5">
                                                                ...
                                                                {lead.lokacija && (
                                                                    <span className="text-white/30">· {lead.lokacija}</span>
                                                                )}
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <td className="p-4">
                                                    {editingStatusId === lead.id ? (
                                                        ...
                                                        <Badge
                                                            ...
                                                            className={cn(
                                                                "px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border cursor-pointer hover:brightness-125 transition-all",
