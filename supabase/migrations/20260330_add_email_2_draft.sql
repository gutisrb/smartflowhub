-- Add email_2_draft column for follow-up email drafts
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS email_2_draft TEXT;

-- Also add reply_intent if not already there
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS reply_intent TEXT;

-- Recreate kontakti view with all current columns
-- (replaces the broken view from 20260327 which referenced id_razgovora)
...
BEGIN
    ...
    INSERT INTO contacts (
        ...
        email_draft, email_2_draft, comment, problem, kategorija, prioritet_skor,
        obrazlozenje, sledeca_akcija, status, izvor, intake_data, client_id,
        email_type, next_email_at, demo_preference, demo_time, meeting_time,
        meeting_link, email_1_poslat, email_2_poslat, email_3_poslat, email_4_poslat,
        email_5_poslat, email_6_poslat, email_7_poslat, email_8_poslat,
        nurture_2_poslat, intake_nudge_poslat, email_framework,
        replied_at, reply_snippet, reply_intent
    ) VALUES (
        new_company_id, NEW.ime, NEW.email, NEW.telefon, NEW.company_name, NEW.niche,
        NEW.service, NEW.website, NEW.instagram_handle, NEW.instagram_followers,
        NEW.website_summary, NEW.last_sent_at, NEW.email_draft, NEW.email_2_draft,
        NEW.comment, NEW.problem, NEW.kategorija, COALESCE(NEW.prioritet_skor, 0),
        NEW.obrazlozenje, NEW.sledeca_akcija, COALESCE(NEW.status, 'Lead'), NEW.izvor,
        NEW.intake_data, NEW.client_id, NEW.email_type, NEW.next_email_at,
        NEW.demo_preference, NEW.demo_time, NEW.meeting_time, NEW.meeting_link,
        COALESCE(NEW.email_1_poslat, false), COALESCE(NEW.email_2_poslat, false),
        COALESCE(NEW.email_3_poslat, false), COALESCE(NEW.email_4_poslat, false),
        COALESCE(NEW.email_5_poslat, false), COALESCE(NEW.email_6_poslat, false),
        COALESCE(NEW.email_7_poslat, false), COALESCE(NEW.email_8_poslat, false),
        COALESCE(NEW.nurture_2_poslat, false), COALESCE(NEW.intake_nudge_poslat, false),
        COALESCE(NEW.email_framework, '3-sentence'), NEW.replied_at, NEW.reply_snippet,
        NEW.reply_intent
    );
...
BEGIN
    UPDATE contacts SET
        ...
        email_2_draft       = NEW.email_2_draft,
        comment             = NEW.comment,
        problem             = NEW.problem,
        ...
        email_framework     = NEW.email_framework,
        replied_at          = NEW.replied_at,
        reply_snippet       = NEW.reply_snippet,
        reply_intent        = NEW.reply_intent,
        updated_at          = NOW()
    WHERE id = NEW.id;
